...
module.exports = {
  ...
  like: async (req, res) => {
    const { commentId, User_Id, isLiked } = req.body;
    const likeCommentRes = await likeComment(commentId, User_Id, isLiked);
    res.status(200).json({
      message: likeCommentRes ? likeCommentRes : "got nothing",
    });
  },
};
